...
def GetPostDate(spanTag):
    ...
    regexGroup = re.search(
        "(\w\w\w\s\d+\s\d\d\d\d),\s(\d\d:\d\d\s\w\w)", cleanedSpanTag
    )
...
def GetBio(threadID):
    ...
    div_bio = soup.find("div", attrs={"class": re.compile("postcolor")})
    bio_text = str(div_bio.contents)
    print(bio_text)
    clean_info = strip_tags(bio_text)
    print("Printing the cleaned info below: ")
    print(clean_info)
